...
use airbug_bench::{image::RgbaImage, *};
...
fn main() -> Result<()> {
    let mut o = Options {
        filter: String::new(),
        goldens: PathBuf::from(".airbug-bench/forma-goldens"),
        record: false,
        tolerance: 0,
        percent: 0.,
        gpu: false,
...
fn surface(r: &forma_vector::gpu::Renderer, w: u32, h: u32, dpi: f32) -> Surface {
    let texture = r.device.create_texture(&wgpu::TextureDescriptor {
        label: Some("bench-golden-target"),
        size: wgpu::Extent3d {
            width: w,
            height: h,
...
async fn run(o: Options) -> Result<()> {
    ...
    let adapter = match instance.request_adapter(&Default::default()).await {
        ...
        Err(e) => {
            ...
            println!("BENCH_RESULT={}", serde_json::to_string(&r.finish()?)?);
            return Ok(());
        }
    };
    if o.cold {
    ...
    for name in SCENARIOS.into_iter().filter(|s| s.contains(&o.filter)) {
        ...
        for pass in 0..if o.record { 1 } else { 2 } {
            ...
            if pass == 1 {
                eprintln!("bench: {id} — measuring {N} frames");
            }
