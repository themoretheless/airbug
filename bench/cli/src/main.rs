    ...
    match cli.command {
        ...
        Action::Gate {
            ...
            if code==2 && rows.iter().all(|r|r.decision=="passed"||r.decision=="inconclusive") && !matches!(uncertainty,Uncertainty::Fail) {
                if matches!(uncertainty,Uncertainty::Warn){eprintln!("bench: warning: inconclusive budgets allowed by policy; no proof of equivalence");}return Ok(0);
            }
            return Ok(code);
        }
        Action::Run {
            ...
            if memory {
                ...
                plan.repetitions=1;
                plan.candidate.env.insert("BENCH_MEMORY_OUTPUT".into(), std::env::current_dir()?.join(&out).join("memory.json").to_string_lossy().into());
            ...
            result?;
            if memory { airbug_bench::memory::Profile::load(&out.join("memory.json"))?; }
        }
        Action::ImportForma {
            source,
            output: out,
        ...
        Action::Compare {
            ...
            if check {
                ...
                if rows.iter().any(|r|r.decision==analysis::Decision::Inconclusive){
                    ...
                    if matches!(uncertainty,Uncertainty::Warn){eprintln!("bench: warning: inconclusive comparison allowed by explicit policy; no proof of equivalence");}
                }
            }
        }
        Action::Build {
