...
fn hash(s: &str) -> String {
    airbug_bench::model::hex(&Sha256::digest(s.as_bytes()))
}
pub fn bundle(store: &Path, run: &Path, out: &Path) -> Result<()> {
    ...
    for (name, data) in [
        ...
        ("report.html", airbug_bench::report::html_run(&r)?),
        (
            "notes.json",
...
pub fn context(a: &Run, b: &Run) -> String {
    ...
    for d in context_diff(a, b) {
        lines.push(format!(
            "\n- {}\n  A: {}\n  B: {}",
            airbug_bench::report::escape(&d.key),
            airbug_bench::report::escape(d.a.as_deref().unwrap_or("<missing>")),
            airbug_bench::report::escape(d.b.as_deref().unwrap_or("<missing>"))
        ));
    }
    if lines.len() == 1 {
...
pub fn trend_points(store: &Path, case: &str, metric: &str) -> Result<Vec<TrendPoint>> {
        ...
        .filter(|h| h.status == "Complete")
    {
        ...
        let medians: Vec<_> = processes
            .values()
            .map(|v| airbug_bench::analysis::median(v))
            .collect();
        let median = if unavailable || medians.is_empty() {
            None
        } else {
            Some(airbug_bench::analysis::median(&medians))
        };
...
pub fn trend(store: &Path, case: &str, metric: &str) -> Result<String> {
    ...
    for p in trend_points(store, case, metric)? {
        ...
        out.push_str(&format!(
            "| {} | {} | {} | {} | {} |\n",
            airbug_bench::report::escape(&p.id),
            airbug_bench::report::escape(p.revision.as_deref().unwrap_or("unrecorded")),
            value,
            airbug_bench::report::escape(&p.unit),
            p.context
        ));
    }
...
pub fn trend_html(markdown: &str) -> String {
    ...
    airbug_bench::report::html(markdown).replace("<!--CHARTS-->", &charts)
}
