...
pub fn run(mut plan: Plan, out: &Path) -> Result<Run> {
    ...
    result.notes.push("Sequential processes; AB/BA order balanced. Whole-process wall includes startup and polling (1 ms). Global lease covers bench only; thermals, frequency and other applications are uncontrolled. Inherited environment is not fully captured; use explicit plan.env for relevant settings.".into());
    result
        .provenance
        ...
        for e in schedule {
            ...
            if plan.protocol {
                ...
                let messages: Vec<_> = text
                    .lines()
                    .filter_map(|l| l.strip_prefix("BENCH_RESULT="))
                    .collect();
                if messages.len() != 1 {
                    return Err(error("expected exactly one BENCH_RESULT message"));
                }
    ...
    if execution.is_ok() {
        if let Some(path) = plan.candidate.env.get("BENCH_MEMORY_OUTPUT") {
            match airbug_bench::memory::Profile::load(Path::new(path)) {
                Ok(_) => {
                    result
                        .provenance
...
pub(crate) fn acquire_lease() -> Result<Lease> {
    // Per-user lock coordinates local bench processes across output directories.
    let uid = std::env::var("USER")
        .unwrap_or_else(|_| "default".into())
        .replace(|c: char| !c.is_ascii_alphanumeric(), "_");
    let lock = std::env::temp_dir().join(format!("bench-{uid}.lock"));
