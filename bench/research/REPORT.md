# Исследование для собственной benchmark-библиотеки bench
...
Повторение сбора: `python3 bench/research/scripts/collect.py`, затем `screen.py`, `curate.py`, `finalize.py`. Сбор использует публичные endpoints, без выполнения скачанного кода. Скрипт архивов `focus.py` — отдельный шаг. Private Forma копировалась через авторизованный GitHub CLI и исключена из будущего Git содержимого bench.
...
6. **Скорость, ресурсы и корректность не образуют один score.** Быстрее с худшим AA, другим input token stream или исключённым transfer — другой эксперимент. bench нужен validity gate, а не только p-value.
