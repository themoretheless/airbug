...
| 11 | Атрибуты | Опциональный feature `macros`: `#[airbug_bench::bench]` генерирует `register_NAME(&mut suite)`. Без автозапуска/скрытого registry; builder остаётся источником case IDs. |
...
```rust
// Cargo.toml: bench = { path = ".../bench", features = ["macros"] }
#[airbug_bench::bench]
fn parsing() -> usize { std::hint::black_box("12345").len() }

fn main() -> airbug_bench::Result<()> {
    let mut suite = airbug_bench::Suite::new("app");
...
```rust
use airbug_bench::workloads::{LocalExecutor, Cancellation, pipeline, parallel};
fn main() -> airbug_bench::Result<()> {
let mut suite = airbug_bench::Suite::new("requests");
...
```sh
cargo airbug-bench diagnose .airbug-bench/run
cargo airbug-bench pilot .airbug-bench/pilot --precision 5 --max-processes 200
cargo airbug-bench multi .airbug-bench/abc --reference baseline
cargo airbug-bench deadlines .airbug-bench/forma --case forma/static --metric frame.completed
cargo airbug-bench git-compare --repo . --baseline BASE --candidate HEAD \
  --target package/bench --offline -o .airbug-bench/pr
cargo airbug-bench pr-report .airbug-bench/pr/run -o .airbug-bench/pr/comment.md
cargo airbug-bench profile .airbug-bench/run --case suite/case --profiler /usr/bin/time \
  -o .airbug-bench/profile -- -l
cargo airbug-bench resume .airbug-bench/interrupted -o .airbug-bench/continued
cargo airbug-bench retention --keep 20
cargo airbug-bench retention --keep 20 --apply
```
...
{
  "plan": {
    "candidate": {"path":"/absolute/bench-forma-example", "args":["--goldens","/absolute/goldens","--filter","static"], "cwd":null},
...
```sh
cargo airbug-bench matrix --plan matrix.json -o .airbug-bench/matrix
cargo build --manifest-path bench/integrations/forma/Cargo.toml --release --features window
cargo airbug-bench run --program bench/integrations/forma/target/release/bench-forma-window \
  --protocol --repetitions 1 --timeout-ms 35000 -o .airbug-bench/window -- --json
```
...
- `.airbug-bench/final20-metal-final`: финальный smoke с 576 normal + 576 GPU frames; draw-call mapping проверен, отсутствующие rebuild/cache counters явно Unsupported.
- `.airbug-bench/final20-allocator-v2`: system / static tracking allocator / active phase, 12 balanced repetitions (36 processes). По 5% margin оба instrumentation comparisons Inconclusive; стоимость не объявлена нулевой и нет автоматического добора до значимости.
- `.airbug-bench/final20-profile-forma`: выбран только `forma/static` из GPU-enabled source; обычный replay под time проходит проверку полного descriptor.
- `.airbug-bench/final20-profile-final`: повтор с финальной проверкой полного descriptor и hashes, 8 samples одного case.
- `.airbug-bench/final20-metal`: 2 × 6 × 96 = **1 152** normal frames и столько же отдельных GPU query samples; checkpoint golden checks. Static: 0 geometry/tile uploads, 96 renderer frames и 4 608 payload bytes за phase.
- `.airbug-bench/final20-window-v2`: **96** measured frames, 3 resize events, 1 occlusion event, 1 skipped frame/acquire; настоящее Metal surface. Первый прогон выявил лишний queued redraw после exit; исправлен верхний предел кадров.
- `.airbug-bench/final20-cold`: 2 новых процесса, первый текстовый кадр, без предыдущего draw; post-golden validation.
- `.airbug-bench/final20-matrix`: 4 Metal combinations с golden equality + 4 Unsupported DX12. Четыре reference PNG визуально просмотрены.
- `.airbug-bench/final20-bisect`: реальная контрольная задержка 1 → 3 ms, правильно найден `5ad2b329a335e1d804e7fec934f82e8031c2fd1d`; 12 независимых process pairs.
- `.airbug-bench/final20-nonmonotonic`: synthetic protocol fixture 100 → 200 → 100; оба commits проверены против reference, результат `nonmonotonic`, culprit `null`.
- `.airbug-bench/final20-profile-v2`: настоящий `/usr/bin/time -l` replay; первый sandbox-прогон честно Failed из-за запрета `sysctl kern.clockrate`.
