...
Статус: **текущая реализация** живёт модулями внутри crates `airbug-bench` + `cargo-airbug-bench` (+ optional `airbug-bench-macros`). Вынос в отдельные packages (`bench-model` / `bench-runner` / …) — возможное будущее, не обязательный следующий шаг. Не создавать пустые crates заранее.

Основной выбор — небольшая Rust-библиотека описания экспериментов, CLI-runner и общий протокол для microbenchmark, процессов и сценариев приложения.
...
Предлагаемые границы **модулей** (сегодня) / packages (позже, по необходимости):
